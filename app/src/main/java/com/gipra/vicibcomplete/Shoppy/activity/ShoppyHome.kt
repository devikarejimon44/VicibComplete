...
//        }
    ...
    private fun getCartList(loginId: String) {
        val stringRequest =
            object : StringRequest(Method.POST, MySingleton.webService + "View_Cart_List",
                Response.Listener<String> { response ->
                    try {
                        val objects = JSONObject(response)
                        //  Log.e(TAG,response)

                        if (objects.getString("status") == "1") showCartStatus(objects)
                        else standardBottomSheet.visibility = View.GONE
